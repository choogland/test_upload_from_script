...
update_jhu = function(input_df, tag) {
  ...
  input_df$Country[input_df$Country=="Cruise Ship" | input_df$Country=="Diamond Princess"] = "Diamond Princess Cruise Ship"
  input_df$Country[str_detect(input_df$Country,"Gambia")] = "TheGambia"
  input_df$Country[str_detect(input_df$Country,"Bahamas")] = "TheBahamas"
  input_df$Country[input_df$Country=="Cabo Verde"] = "Cape Verde"
  input_df$Country[input_df$Country=="Syria"] = "SyrianArabRepublic"
  input_df$Country[input_df$Country=="Timor-Leste" | input_df$Country=="East Timor"] = "EastTimor"
  input_df$Country[str_detect(input_df$Country,"West Bank and Gaza")] = "occupiedPalestinianterritory"
  input_df$Country[input_df$Country=="Guinea-Bissau"] = "GuineaBissau"
  input_df$Country[input_df$Country=="Burma"] = "Myanmar"
  input_df$Country[input_df$Country=="Tanzania, United Republic of"] = "United Republic of Tanzania"
  input_df$Country[input_df$Country=="Moldova, Republic of"] = "Republic of Moldova"
...
jhu_cases_ori <- as.data.frame(data.table::fread("https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_confirmed_global.csv"))
total_cases_ori <- sum(jhu_cases_ori[,ncol(jhu_cases_ori)])

jhu_cases = update_jhu(jhu_cases_ori, "cases")
jhu_cases[is.na(jhu_cases)]=0
#write.,"_cases",""), "input_data/jhu_names.csv")
if (total_cases_ori!=sum(jhu_cases[nrow(jhu_cases),1:(ncol(jhu_cases)-1)])) { 
...
jhu_deaths_ori <- as.data.frame(data.table::fread("https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_deaths_global.csv"))
total_deaths_ori <- sum(jhu_deaths_ori[,ncol(jhu_deaths_ori)])

jhu_deaths = update_jhu(jhu_deaths_ori, "deaths")
jhu_deaths[is.na(jhu_deaths)]=0
if (total_deaths_ori!=sum(jhu_deaths[nrow(jhu_deaths),1:(ncol(jhu_deaths)-1)])) { 
  stop(paste0("Error: incorrect processing - total counts do not match"))
}
...
#check countries for cases  and deaths are matching
jhu_country_cases = names(jhu_merge)[grepl("_cases", names(jhu_merge))] %>% str_replace_all(., "_cases", "") 
jhu_country_deaths = names(jhu_merge)[grepl("_deaths", names(jhu_merge))] %>% str_replace_all(., "_deaths", "") 
#jhu_country_rec = names(jhu_merge)[grepl("_recovered", names(jhu_merge))] %>% str_replace_all(., "_recovered", "") 

setdiff(jhu_country_cases, jhu_country_deaths)
#setdiff(jhu_country_rec,jhu_country_cases)
...
countries_without_coordinates = jhu_country_list[(jhu_country_list %in% countries$jhu_ID) == FALSE]
...
# update jhu_merge
for (country in countries_without_coordinates)
{
  cols_to_drop <- c(paste(country, "_cases", sep=""),paste(country, "_deaths", sep=""))
  jhu_merge = jhu_merge[ , !(names(jhu_merge) %in% cols_to_drop)]
}
...
write.csv(collated_data, "www/downloads/coronavirus.csv", row.names=F)
